...
export async function GET(){
 try{const u=await requireUser();const user=await prisma.user.findUnique({where:{id:u.id},select:{id:true,name:true,email:true,image:true,bio:true,avatarBorder:true,role:true,createdAt:true}});return user?NextResponse.json(user):NextResponse.json({error:"User tidak ditemukan."},{status:404})}
...
export async function PATCH(req:Request){
 try{
  ...
  if(Object.prototype.hasOwnProperty.call(d,"bio")){const bio=String(d.bio||"").trim();if(bio.length>1000)return NextResponse.json({error:"Bio maksimal 1000 karakter."},{status:400});data.bio=bio;}
  ...
  if(Object.prototype.hasOwnProperty.call(d,"avatarBorder")){const border=String(d.avatarBorder||"solid");if(!["solid","none"].includes(border))return NextResponse.json({error:"Avatar border tidak valid."},{status:400});data.avatarBorder=border;}
