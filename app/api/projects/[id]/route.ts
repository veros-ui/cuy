...
export async function GET(_:Request,{params}:{params:{id:string}}){
 try{
  const p=await prisma.project.findUnique({where:{id:params.id},include:{owner:{select:{id:true,name:true,email:true,image:true,bio:true,role:true}},reviews:{include:{user:{select:{name:true,image:true}}},orderBy:{createdAt:"desc"}}}});
